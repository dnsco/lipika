...
# a trace under `reference/` is never edited

Cross-check on `trace-names-are-dated`'s immutability clause. A record is corrected by a newer dated document, never by a
...
This is a cheap proxy, not the rule: `trace-names-are-dated`'s judged clause is what scores the behaviour.
